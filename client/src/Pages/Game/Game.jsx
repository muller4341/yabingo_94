...
const Game = () => {
  ...
  // IMPROVED AUDIO PRELOADING with better error handling and loading states
  useEffect(() => {
    const preloadAudio = async () => {
      ...
      try {
        ...
        // Preload number audio files with better error handling
        ...
        for (let i = 1; i <= 75; i++) {
          ...
          const promise = new Promise((resolve) => {
            ...
            const onError = () => {
              console.warn(`Failed to load audio for ${filename}`)
              resolve(false)
            }
        ...
        const controlAudios = {
          play: "https://firebasestorage.googleapis.com/v0/b/blog-aeffd.appspot.com/o/audio%2Fp.mp3?alt=media&token=d558540b-c563-48c0-a22a-5fec63fa8df9",
          continue:
            "https://firebasestorage.googleapis.com/v0/b/blog-aeffd.appspot.com/o/audio%2Fc.mp3?alt=media&token=your-continue-token",
          stop: "https://firebasestorage.googleapis.com/v0/b/blog-aeffd.appspot.com/o/audio%2Fs.mp3?alt=media&token=your-stop-token",
          shuffle:
            "https://firebasestorage.googleapis.com/v0/b/blog-aeffd.appspot.com/o/audio%2Fsh.mp3?alt=media&token=your-shuffle-token",
          winner:
            "https://firebasestorage.googleapis.com/v0/b/blog-aeffd.appspot.com/o/audio%2Fw.mp3?alt=media&token=your-winner-token",
          try: "https://firebasestorage.googleapis.com/v0/b/blog-aeffd.appspot.com/o/audio%2Ft.mp3?alt=media&token=your-try-token",
          notFound:
            "https://firebasestorage.googleapis.com/v0/b/blog-aeffd.appspot.com/o/audio%2Fn.mp3?alt=media&token=your-notfound-token",
        }
        ...
        const controlPromises = Object.entries(controlAudios).map(([key, path]) => {
          return new Promise((resolve) => {
            ...
            const onError = () => {
              console.warn(`Failed to load control audio for ${key}`)
              resolve(false)
            }
        ...
        await Promise.all([...numberPromises, ...controlPromises])
  ...
  const playControlAudio = (type) => {
    try {
      ...
      if (audio && audio.readyState >= 2) {
        ...
        console.warn(`Control audio ${type} not ready or not loaded`)
      }
    } catch (error) {
  ...
  const playNumberAudio = (number) => {
    try {
      ...
      if (audio && audio.readyState >= 2) {
        ...
        console.warn(`Number audio ${number} not ready or not loaded`)
      }
    } catch (error) {
          ...
          <div className="flex-col flex-1 w-full max-w-md mx-auto bg-gradient-to-r from-fuchsia-200 via-yellow-100 to-green-200 rounded-xl shadow-lg p-4 border-2 border-fuchsia-300">
            <div className="flex flex-col md:flex-row items-center justify-center w-full gap-2">
              ...
              <button
                ...
                type="button"
                onClick={isPlaying ? stopGame : startGame}
                disabled={!audioLoadedRef.current}
              >
                {isPlaying ? (
                  <>
                    <svg
                  ...
                  <>
                    ...
                    {audioLoadedRef.current ? "Play" : "Loading..."}
                  </>
                )}
              </button>
              <button
