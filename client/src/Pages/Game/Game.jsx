...
//       // Play clap 0.5s before winnerAudio ends
...
// //   const [pendingSubmissions, setPendingSubmissions] = useState([])
// //   const [showOfflineStatus, setShowOfflineStatus] = useState(false)
// //   const [searchedCartelas, setSearchedCartelas] = useState({}) // { [cartelaNumber]: true }
...
// //     checkAndLockCartelas(nextNumber)

// //     return nextNumber
// //   }

// //   const checkAndLockCartelas = (calledNumber) => {
// //     if (!recent?.cartelas || !Array.isArray(recent.cartelas)) return

// //     const cartelasToLock = {}

// //     recent.cartelas.forEach((cartelaObj) => {
// //       const cartelaNumber = String(cartelaObj.cartelaNumber)

// //       // Skip if already locked or if it's a winner
// //       if (lockedNonWinners[cartelaNumber]) return
// //       if (checkBingoWin(cartelaObj.grid, [...calledNumbersRef.current, calledNumber])) return

// //       // Check if the called number exists in this cartela
// //       let numberFoundInCartela = false
// //       let numberInWinningPattern = false

// //       for (let row = 0; row < 5; row++) {
// //         for (let col = 0; col < 5; col++) {
// //           const cellValue = cartelaObj.grid[row][col]

// //           if (cellValue === calledNumber) {
// //             numberFoundInCartela = true

// //             // Get all winning patterns for this cartela
// //             const allWinningPatterns = getWinningPattern(cartelaObj.grid, [...calledNumbersRef.current, calledNumber])

// //             // Check if this cell is part of any winning pattern
// //             if (isCellInWinningPattern(row, col, allWinningPatterns)) {
// //               numberInWinningPattern = true
// //               break
// //             }
// //           }
// //         }
// //         if (numberInWinningPattern) break
// //       }

// //       // If number is found in cartela but NOT in winning pattern, lock it
// //       if (numberFoundInCartela && !numberInWinningPattern) {
// //         cartelasToLock[cartelaNumber] = true
// //         console.log(
// //           `[v0] Auto-locking cartela ${cartelaNumber} - number ${calledNumber} found but not in winning pattern`,
// //         )
// //       }
// //     })

// //     // Update locked cartelas if any need to be locked
// //     if (Object.keys(cartelasToLock).length > 0) {
// //       setLockedNonWinners((prev) => ({
// //         ...prev,
// //         ...cartelasToLock,
// //       }))
...
// //     const isFirstTimeSearch = !searchedCartelas[cartelaNumber]
// //     const isAlreadyLocked = lockedNonWinners[cartelaNumber]

// //     // If it's second-time search and not a winner, lock automatically
// //     if (!isFirstTimeSearch && !isAlreadyLocked) {
// //       const isWinner = checkBingoWin(cartelaGrid, calledNumbers)
// //       if (!isWinner) {
// //         setLockedNonWinners((prev) => ({
// //           ...prev,
...
// //     if (!isWinner && isFirstTimeSearch) {
// //       try {
// //         const audio = new Audio("/images/Audio/bingo/t.mp3")
// //         audio.play().catch(console.error)
// //       } catch (error) {
// //         console.error("Could not play audio:", error)
// //       }

// //       // Show the cartela value first, then lock after a brief delay
// //       setSearchResult({
// //         cartela: { cartelaNumber: cartelaNumber, grid: cartelaGrid },
// //         isWinner: false,
// //         isLocked: false, // Show unlocked first
// //         notFound: false,
// //       })
// //       setShowPopup(true)

// //       // Lock after showing the cartela
// //       setTimeout(() => {
// //         setLockedNonWinners((prev) => ({
// //           ...prev,
// //           [cartelaNumber]: true,
...
// //           isLocked: true,
// //         }))
// //       }, 2000) // 2 second delay to show cartela before locking
// //       return
// //     }
...
function checkBingoWin(grid, calledNumbers) {
  ...
  for (let i = 0; i < 5; i++) {
    // Horizontal
    const horizontalWin = grid[i].every(
      (val, j) => (val === "FREE" && i === 2 && j === 2) || (typeof val === "number" && calledNumbers.includes(val)),
    )
    if (horizontalWin) {
      // Check if last called number is part of this winning row
      const lastNumberInRow = grid[i].includes(lastCalledNumber)
      if (lastNumberInRow) return true
    }

    // Vertical
    const verticalWin = [0, 1, 2, 3, 4].every((rowIdx) => {
      const cellValue = grid[rowIdx][i]
      return (
        (cellValue === "FREE" && rowIdx === 2 && i === 2) ||
        (typeof cellValue === "number" && calledNumbers.includes(cellValue))
    ...
    if (verticalWin) {
      // Check if last called number is part of this winning column
      const lastNumberInColumn = [0, 1, 2, 3, 4].some((rowIdx) => grid[rowIdx][i] === lastCalledNumber)
      if (lastNumberInColumn) return true
    }
  }

  // 2. Diagonals
  // Main diagonal (top-left to bottom-right)
  const mainDiagonalWin = [0, 1, 2, 3, 4].every(
    (i) => (grid[i][i] === "FREE" && i === 2) || (typeof grid[i][i] === "number" && calledNumbers.includes(grid[i][i])),
  )
  if (mainDiagonalWin) {
    const lastNumberInMainDiagonal = [0, 1, 2, 3, 4].some((i) => grid[i][i] === lastCalledNumber)
    if (lastNumberInMainDiagonal) return true
  }

  // Anti-diagonal (top-right to bottom-left)
  const antiDiagonalWin = [0, 1, 2, 3, 4].every(
    (i) =>
      (grid[i][4 - i] === "FREE" && i === 2) ||
      (typeof grid[i][4 - i] === "number" && calledNumbers.includes(grid[i][4 - i])),
  )
  if (antiDiagonalWin) {
    const lastNumberInAntiDiagonal = [0, 1, 2, 3, 4].some((i) => grid[i][4 - i] === lastCalledNumber)
    if (lastNumberInAntiDiagonal) return true
  }
  ...
  if (marked === 0 && calledNumbers.length >= 15) {
    // For this pattern, any number in the cartela can be the last called number
    // since the win condition is having NO marked numbers
    return true
  }
  ...
  const outerCornersWin = corners.every(
    ([i, j]) => typeof grid[i][j] === "number" && calledNumbers.includes(grid[i][j]),
  )
  if (outerCornersWin) {
    const lastNumberInOuterCorners = corners.some(([i, j]) => grid[i][j] === lastCalledNumber)
    if (lastNumberInOuterCorners) return true
  }
  ...
  const innerCornersWin = inner.every(([i, j]) => typeof grid[i][j] === "number" && calledNumbers.includes(grid[i][j]))
  if (innerCornersWin) {
    const lastNumberInInnerCorners = inner.some(([i, j]) => grid[i][j] === lastCalledNumber)
    if (lastNumberInInnerCorners) return true
  }
                              ...
                              <div key={rowIdx} className="flex gap-1 justify-center">
                                {row.map((val, colIdx) => {
                                  ...
                                  return (
                                    <div
                                      key={colIdx}
                                      className={`w-16 h-12 flex items-center justify-center rounded-lg font-bold text-lg border-3 transition-all duration-300 ${
                                        isNum
                                          ? isInWinningPattern
                                            ? "bg-gradient-to-br from-green-400 to-green-600 text-white border-green-700 shadow-lg transform scale-105 ring-2 ring-green-300"
                                            : isCalled
